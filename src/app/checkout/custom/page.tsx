...
export default function CustomCheckoutPage() {
  ...
  const proceedToWhatsApp = async () => {
    ...
    // 1. Create Custom Order in DB
    let customOrderId = null;
    try {
      const payload = {
        front_image_url: customItem.front_image_url || null,
        back_image_url: customItem.back_image_url || null,
    ...
    let msg = `*New Custom T-Shirt Order #${customOrderId}*\n`;
    ...
    if (customItem.front_image_url) {
      msg += `- Front Design: ${customItem.front_image_url}\n`;
    }
    if (customItem.back_image_url) {
      msg += `- Back Design: ${customItem.back_image_url}\n`;
    }
    if (customItem.instructions) {
      msg += `\n*Instructions:*\n${customItem.instructions}\n`;
    }
    ...
    // Open WhatsApp in a new tab and keep this page available so we can redirect to success
    try {
    ...
    setTimeout(() => {
      router.replace(`/checkout/success?orderId=${customOrderId}&type=custom`);
    }, 1000);
  };
              ...
              <div className="mb-6">
                <div className="flex gap-4 mb-4">
                  <div className="relative w-20 h-20 bg-gray-50 rounded-lg overflow-hidden shrink-0 border border-gray-100">
                    <Image
                      src={customItem.front_image_url || customItem.back_image_url || '/glee_logo.png'}
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {customItem.front_image_url && (
                    <div className="bg-gray-50 rounded-lg p-2 border border-gray-100">
                      <p className="text-[10px] font-bold text-gray-500 uppercase mb-2">Front Design</p>
                      <a href={customItem.front_image_url} target="_blank" rel="noopener noreferrer" className="block relative aspect-square rounded overflow-hidden hover:opacity-80 transition-opacity">
                        <Image src={customItem.front_image_url} alt="Front design" fill className="object-contain" unoptimized />
                      </a>
                    </div>
                  )}
                  {customItem.back_image_url && (
                    <div className="bg-gray-50 rounded-lg p-2 border border-gray-100">
                      <p className="text-[10px] font-bold text-gray-500 uppercase mb-2">Back Design</p>
                      <a href={customItem.back_image_url} target="_blank" rel="noopener noreferrer" className="block relative aspect-square rounded overflow-hidden hover:opacity-80 transition-opacity">
                        <Image src={customItem.back_image_url} alt="Back design" fill className="object-contain" unoptimized />
                      </a>
                    </div>
                  )}
                </div>
                ...
                {customItem.instructions && (
                  <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                    <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">Design Instructions</p>
                    <p className="text-xs text-gray-700 leading-relaxed">{customItem.instructions}</p>
                  </div>
                )}
              </div>
